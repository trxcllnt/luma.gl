"use strict";var module1=module;module1.export({setShaderHook:()=>setShaderHook,setModuleInjection:()=>setModuleInjection,assembleShaders:()=>assembleShaders});var VERTEX_SHADER,FRAGMENT_SHADER;module1.link('./constants',{VERTEX_SHADER(v){VERTEX_SHADER=v},FRAGMENT_SHADER(v){FRAGMENT_SHADER=v}},0);var resolveModules,getShaderModule;module1.link('./resolve-modules',{resolveModules(v){resolveModules=v},getShaderModule(v){getShaderModule=v}},1);var getPlatformShaderDefines,getVersionDefines;module1.link('./platform-defines',{getPlatformShaderDefines(v){getPlatformShaderDefines=v},getVersionDefines(v){getVersionDefines=v}},2);var injectShader;module1.link('./inject-shader',{default(v){injectShader=v}},3);var assert;module1.link('../utils',{assert(v){assert=v}},4);
...
function setShaderHook(type, opts) {
  const name = opts.signature.trim().replace(/\(.+/, '');
  HOOK_FUNCTIONS[type][name] = opts;
}

function setModuleInjection(moduleName, opts) {
...
`;
  ...
  const injectionPrefix = INJECTION_PREFIXES[type];
  ...
  for (const key in inject) {
    ...
    if (key.indexOf(injectionPrefix) === 0) {
