"use strict";var createGLContext;module.link('@luma.gl/core',{createGLContext(v){createGLContext=v}},0);var assembleShaders;module.link('@luma.gl/shadertools',{assembleShaders(v){assembleShaders=v}},1);var injectShader,combineInjects;module.link('@luma.gl/shadertools/lib/inject-shader',{default(v){injectShader=v},combineInjects(v){combineInjects=v}},2);var test;module.link('tape-catch',{default(v){test=v}},3);/* eslint-disable camelcase, no-console, no-undef */
...
test.only('injectShader#injectShader', t => {
  ...
  t.ok(
    injectResult.replace(/\s/g, '').indexOf(VS_GLSL_RESOLVED_DECL.replace(/\s/g, '')) > -1,
    'declarations correctly injected in vertex shader'
  );
  t.ok(
    injectResult.replace(/\s/g, '').indexOf(VS_GLSL_RESOLVED_MAIN.replace(/\s/g, '')) > -1,
    'main correctly injected in vertex shader'
  );

  injectResult = injectShader(FS_GLSL_TEMPLATE, 'fs', INJECT);
  t.ok(
    injectResult.replace(/\s/g, '').indexOf(FS_GLSL_RESOLVED_DECL.replace(/\s/g, '')) > -1,
    'declarations correctly injected in vertex shader'
  );
  t.ok(
    injectResult.replace(/\s/g, '').indexOf(FS_GLSL_RESOLVED_MAIN.replace(/\s/g, '')) > -1,
    'main correctly injected in vertex shader'
  );
  console.log(injectResult);
  console.log(FS_GLSL_RESOLVED_MAIN);
...
test('injectShader#assembleShaders', t => {
  ...
  t.ok(
    assembleResult.vs.replace(/\s/g, '').indexOf(VS_GLSL_RESOLVED_DECL.replace(/\s/g, '')) > -1,
    'declarations correctly assembled in vertex shader'
  );
  t.ok(
    assembleResult.vs.replace(/\s/g, '').indexOf(VS_GLSL_RESOLVED_MAIN.replace(/\s/g, '')) > -1,
    'main correctly assembled in vertex shader'
  );

  t.ok(
    assembleResult.fs.replace(/\s/g, '').indexOf(FS_GLSL_RESOLVED_DECL.replace(/\s/g, '')) > -1,
    'declarations correctly assembled in vertex shader'
  );
  t.ok(
    assembleResult.fs.replace(/\s/g, '').indexOf(FS_GLSL_RESOLVED_MAIN.replace(/\s/g, '')) > -1,
    'main correctly assembled in vertex shader'
  );
