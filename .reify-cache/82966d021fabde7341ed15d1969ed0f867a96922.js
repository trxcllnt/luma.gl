"use strict";module.export({default:()=>Transform});var GL;module.link('@luma.gl/constants',{default(v){GL=v}},0);var combineInjects;module.link('@luma.gl/shadertools',{combineInjects(v){combineInjects=v}},1);var transform,getPassthroughFS,typeToChannelCount;module.link('@luma.gl/shadertools',{_transform(v){transform=v},getPassthroughFS(v){getPassthroughFS=v},typeToChannelCount(v){typeToChannelCount=v}},2);var isWebGL2,Buffer,Framebuffer,Texture2D,TransformFeedback,readPixelsToArray,getShaderVersion,cloneTextureFrom;module.link('@luma.gl/webgl',{isWebGL2(v){isWebGL2=v},Buffer(v){Buffer=v},Framebuffer(v){Framebuffer=v},Texture2D(v){Texture2D=v},TransformFeedback(v){TransformFeedback=v},readPixelsToArray(v){readPixelsToArray=v},getShaderVersion(v){getShaderVersion=v},cloneTextureFrom(v){cloneTextureFrom=v}},3);var log,isObjectEmpty,assert;module.link('../utils',{log(v){log=v},isObjectEmpty(v){isObjectEmpty=v},assert(v){assert=v}},4);var Model;module.link('./model',{default(v){Model=v}},5);var updateForTextures,getSizeUniforms;module.link('./transform-shader-utils',{updateForTextures(v){updateForTextures=v},getSizeUniforms(v){getSizeUniforms=v}},6);
